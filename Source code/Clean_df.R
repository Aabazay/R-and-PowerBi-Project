...
#View(df) #dim-used to get or set the dimension of the specified matrix, array or data frame.
dim(df)
...
ggplot(df,aes(x=Profitability, y=Worldwide.Gross)) +geom_boxplot(outlier.colour= "blue",outlier.shape= 5)+scale_x_continuous(labels = scales::comma)+coord_cartesian(ylim= c(0,1000))
...
write_excel_csv(df1, "clean_df.xl")
